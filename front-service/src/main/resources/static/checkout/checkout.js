angular.module('store').controller('checkoutController', function ($scope, $http, $location, $localStorage) {
    ...
    $scope.loadCart = function () {
        var id = $localStorage.contextPath + '/cart/api/v1/cart/' + $localStorage.webMarketGuestCartId
        console.log(id)
        $http({
            url: $localStorage.contextPath + '/cart/api/v1/cart/' + $localStorage.webMarketGuestCartId,
            method: 'GET'
