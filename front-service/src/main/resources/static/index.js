(function () {
    ...
    function run($rootScope, $http, $localStorage) {
        $localStorage.contextPath = 'http://127.0.0.1:5555';
        if ($localStorage.webMarketUser) {
            $http.defaults.headers.common.Authorization = 'Bearer ' + $localStorage.webMarketUser.token;
        }
        console.log($localStorage.contextPath + '/cart/api/v1/cart/generate');
        if (!$localStorage.webMarketGuestCartId) {
            $http.get($localStorage.contextPath + '/cart/api/v1/cart/generate')
    ...
    angular.module('store').controller('indexController', function ($rootScope, $scope, $http, $localStorage) {
        ...
        $scope.tryToAuth = function () {
            $http.post($localStorage.contextPath + '/auth/api/v1/auth', $scope.user)
                .then(function successCallback(response) {
